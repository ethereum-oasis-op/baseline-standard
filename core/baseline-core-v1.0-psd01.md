...
An interface connecting and synchronizing a Baseline Protocol Implementation (BPI) and System of Record.
...
A concretization of the above used in this document: Identity is the combination of one or more unique identifiers with data associated with this/these identifi-er(s). Identity associated data consists of signed certificates or credentials such as veri-fiable credentials and other unsigned, non-verifiable data objects generated by or on be-half of the unique identifier(s).
...
A way of ensuring the privacy of Workflow data represented on a DLT.
...
A peer-to-peer network is required as well as a consensus algorithm to ensure replication across nodes is undertaken (see also the Glossary). 
...
**[R28]**	DLTs used in the implementation of a BPI MUST support bilateral and multi-lateral digital representations of legal contracts.

**[R29]**	A DLT or BPI Abstraction Layer used in a BPI MUST support more than one DLT instance.

**[R30]** A DLT or BPI Abstraction Layer used in a BPI MUST support more than one DLT type.
...
<figure>
  <img
  src="./images/Baseline-Fig-6.1-Workflow-Agreement-Excution.png"
  >
...
See sections [6.4](#64-BPI-Accounts) and [6.5]((#65-BPI-Transactions)) below on BPI accounts associated with state objects and BPI transactions for details.
...
**[RXX]** The cryptographic proof of the DLT commitments MUST be verifiable by any 3rd party at any time in a time at most proportional to the size of the prover system. 
...
**[DXX]** The cryptographic proof of the DLT commitment SHOULD be verifiable by any 3rd party at any time on the DLT utilized by the BPI.
...
This means that requirements such as determinism, updatability, versioning, characteristics of zero-knowledge proofs etc. also apply to a workflow.
...
**[CRXX1]>[DXX1]** The prover system in a workflow MUST be the same for all worksteps.

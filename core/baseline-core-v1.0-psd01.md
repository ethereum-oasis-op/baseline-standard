...
Oracle data, since it resides outside of an authoritative record, is subject to manipulation risks that other data is not subject to. As such, steps should be taken to remove counterparty manipulation and error risk through mechanisms such as redundancy in the data reporting and error checking, either cross-party, by a neutral party or an honesty incentivized party. Effectively, the principles of decentralization securing CCSMs should be carried over where possible for external oracle data so that input to a BPI workstep can generate deterministic results.

**[D34]**	Where possible, external non-authoritative BPI input data SHOULD be sourced from multiple endpoints.

**[R320]**	External non-authoritative BPI workstep input data MUST be validated by one or more authoritative entities.

...
[[R213]](#r213) Testability: A verification system comprised of rules can be expressed in computer code. And computer code is testable given an input and expected output.   
...
[[R214]](#r214) Testability: A verifiable validation result as the verifiable validation result of an input into a workstep as a correct or incorrect new agreement state as an output of the workstep can be expressed in code as a testable, and verifiable, zero-knowledge proof of correctness of the new state for example [a zk-snark proof and its validation generated by GNARK](https://github.com/ConsenSys/gnark/tree/master/test) if the proposed new state is correct or an error message if such a proof cannot be constructed, the requirement is testable. 

#### **[R215]** 
A workstep instance MUST be associated with only one workgroup.

[[R215]](#r215) Testability: Creating and assigning a workstep and a workgroup unique identifiers can be done in code and is, therefore, testable. The association of a those two unique identfiers can be expressed in for example a table in a data base and is, therefore, testable. Code-based data base table update procedures such as SQL procedures can be dressed with logical qualifiers such as where the combination of both unique identifiers cannot already be present in a given table that assure uniqueness of an identifier pair in a given table are testable. Therefore, the requirement as a combination of the previous testable statements is testable.     
...
[[R216]](#r216) Testability: Since [[R215]](#r215) expresses the testable association of a workstep and a workgroup, and since policies such as an authentication requirement of a BPI Subject are expressible as evaluative functions in code and, therefore, testable, and since policies can be logically associated with a workgroup as a workstep can be associated with a workgroup, a workgroup policy can be evaluated in the context of a workstep in a testable manner, and, therefore, the requirement is testable.  
